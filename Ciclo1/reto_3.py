...
def clima(datos):
    ...
    array=np.array(datos).reshape(int(len(datos)/7),7)
    ...
    fechas_max_dias_sol=[]
    fecha = array[:,0]
    tempMinPro = array[:,1].astype(dtype=float)
    tempMaxPro = array[:,2].astype(dtype=float)
    preciTotal = array[:,3].astype(dtype=int)
    cantDiasSol = array[:,6].astype(dtype=int)
